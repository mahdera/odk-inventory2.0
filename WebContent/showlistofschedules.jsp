	...
	<%
		...
		while(scheduleItr.hasNext()){
			...
			%>
				...
				<td>
					<a href="#.jsp" onclick="showAddReminderFormForThisSchedule(<%=schedule.getId() %>);">Add</a> |
					<a href="#.jsp" onclick="showListOfRemindersForThisScheduleWithEditOption(<%=schedule.getId() %>);">Edit</a> |
					<a href="#.jsp" onclick="showListOfRemindersForThisScheduleWithDeleteOption(<%=schedule.getId() %>);">Delete</a>
				</td>								
			</tr>
			<%
				String divId = "scheduleReminderDiv"+schedule.getId();
			%>
			<tr>
				<td colspan="6">
					<div id="<%=divId %>"></div>
