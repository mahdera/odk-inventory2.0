...
<%	
	...
	%>
	<h3 style="background:lightyellow">Contraceptive Lost or Damaged Pattern Report For All Health Post During <%=fromDate %> up to <%=toDate %></h3>
	<%
	if(true){
				...
				while(hewManagesHealthPostItr.hasNext()){
					...
					if(hewManagesHealthPost != null){
						...
						if(relatedUser != null){
							...
							float pillsAmt = LostDamaged.getTotalLostOrDamagedAmountsForThisItemByThisUserDuringThisPeriod("PILLS_AMOUNT",relatedUser.get_uri(),fromDate,toDate);
							float condomAmt = LostDamaged.getTotalLostOrDamagedAmountsForThisItemByThisUserDuringThisPeriod("CONDOM_AMOUNT",relatedUser.get_uri(),fromDate,toDate);
							float injectableAmt = LostDamaged.getTotalLostOrDamagedAmountsForThisItemByThisUserDuringThisPeriod("INJECTABLE_AMOUNT",relatedUser.get_uri(),fromDate,toDate);
							float implantAmt = LostDamaged.getTotalLostOrDamagedAmountsForThisItemByThisUserDuringThisPeriod("IMPLANT_AMOUNT",relatedUser.get_uri(),fromDate,toDate);
