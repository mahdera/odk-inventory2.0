...
<%	
	...
	%>
	<h3 style="background:lightyellow">Contraceptive Receive Pattern Report For All Health Post During <%=fromDate %> up to <%=toDate %></h3>
	<%
	if(true){
				...
				while(hewManagesHealthPostItr.hasNext()){
					...
					if(hewManagesHealthPost != null){
						...
						if(relatedUser != null){
							...
							float pillsAmt = GRV.getTotalReceivedAmountsForThisItemByThisUserDuringThisPeriod("PILLS_AMOUNT",relatedUser.get_uri(),fromDate,toDate);
							float condomAmt = GRV.getTotalReceivedAmountsForThisItemByThisUserDuringThisPeriod("CONDOM_AMOUNT",relatedUser.get_uri(),fromDate,toDate);
							float injectableAmt = GRV.getTotalReceivedAmountsForThisItemByThisUserDuringThisPeriod("INJECTABLE_AMOUNT",relatedUser.get_uri(),fromDate,toDate);
							float implantAmt = GRV.getTotalReceivedAmountsForThisItemByThisUserDuringThisPeriod("IMPLANT_AMOUNT",relatedUser.get_uri(),fromDate,toDate);
