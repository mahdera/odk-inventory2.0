...
public class GIV {
	 ...
	 */
	public GIV(String _uri, String _creator_uri_user, Date _creation_date,
	...
	public void addGIV() {
		try {
			String command = "insert into build___1354971908_core values('"+
	...
	public static void updateGIV(String _uri,String _creator_uri_user,
		...
		try {
			String command = "update build___1354971908_core set _CREATOR_URI_USER='"+_creator_uri_user+"', _CREATION_DATE='"+_creation_date+"', _LAST_UPDATE_URI_USER = '"+_last_update_uri_user+"', LAST_UPDATE_DATE = '"+
	...
	public static void deleteGIV(String _uri,String _creator_uri_user,Date _creation_date) {
		try {
			String command = "delete from build___1354971908_core where _URI = '"+_uri+"'";
			DBConnection.writeToDatabase(command);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	public static List<GIV> getAllGIVs() {
		List<GIV> list = new ArrayList<GIV>();
		GIV giv = null;
		try {
			String query = "select * from build___1354971908_core order by _CREATION_DATE desc";
	...
	public static List<GIV> getAllGIVsFilledByODKUser(String odkUser){
		List<GIV> list = new ArrayList<GIV>();
		GIV giv = null;
		try {
			String query = "select * from build___1354971908_core where _CREATOR_URI_USER = '"+odkUser+"'";
	...
	public static GIV getGIVWithURI(String _uri){
		GIV giv = null;
		try {
			String query = "select * from build___1354971908_core where _uri = '"+_uri+"'";
			System.out.println(query);
			ResultSet rSet = DBConnection.readFromDatabase(query);
			while(rSet.next()){
				giv = new GIV(rSet.getString("_URI"), rSet.getString("_CREATOR_URI_USER"),
	...
	public static void updateGIVWithURI(String _uri,float pillsAmount,float condomAmount,
			float injectableAmount,float implantAmount){
		try{
			String command = "update build___1354971908_core set pills_amount="+pillsAmount+
	...
	public static void deleteGIVWithURI(String _uri){
		try{
			String command = "delete from build___1354971908_core where _uri='"+_uri+"'";
			DBConnection.writeToDatabase(command);
		}catch(Exception e){
	...
	public static float getTotalIssuedAmountsForThisItemByThisUserDuringThisPeriod(String itemCode,
			String userURI,Date fromDate,Date toDate){
		float amt=0.0f;
		try{
			String query = "select sum("+itemCode+") as a from build___1354971908_core where _CREATOR_URI_USER = '"+
	...
	public static float getItemIssuedQuantityForThis_URIAndThisItemDuringThisPeriod(String _uri,String itemCode,Date fromDate,Date toDate){
		float qty = 0.0f;
		try{
			String query = "select sum("+itemCode+") as q from build___1354971908_core where _CREATOR_URI_USER = '"+_uri+"' "+
			"and _submission_date between '"+fromDate+"' and '"+toDate+"'";
			System.out.println("GIV: "+query);
			ResultSet rSet = DBConnection.readFromDatabase(query);
			while(rSet.next()){
				qty = rSet.getFloat("q");
			}
	...
	public static float getTheTotalIssuedQuantityForThisItemByThisCreatorUser(String itemCode,String creatorUser){
		float totalQty = 0.0f;
		try{
			String query = "select sum("+itemCode+") as a from build___1354971908_core where _CREATOR_URI_USER = '"+
			creatorUser+"'";
