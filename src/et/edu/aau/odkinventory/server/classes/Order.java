 ...
 * 
 */
public class Order { // build___1354971595_core
	private String _uri;//is the primary key
	 ...
	 */
	public Order(String _uri, String _creator_uri_user, Date _creation_date,
	...
	public void addOrder() {
		try {
			String command = "insert into   build___1354971595_core values('"+
	...
	public static void updateOrder(String _uri,String _creator_uri_user,
		...
		try {
			String command = "update   build___1354971595_core set _CREATOR_URI_USER='"+_creator_uri_user+"', _CREATION_DATE='"+_creation_date+"', _LAST_UPDATE_URI_USER = '"+_last_update_uri_user+"', LAST_UPDATE_DATE = '"+
	...
	public static void deleteOrder(String _uri) {
		try {
			String command = "delete from   build___1354971595_core where _URI = '"+_uri+"'";
			DBConnection.writeToDatabase(command);
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	public static List<Order> getAllOrdersOfThisHealthPost(int healthPostId){
		List<Order> list = new ArrayList<Order>();
		Order order = null;
		try {
			String query = "select * from   build___1354971595_core order by _CREATION_DATE desc";
			ResultSet rSet = DBConnection.readFromDatabase(query);
			while(rSet.next()){
				order = new Order(rSet.getString("_URI"), rSet.getString("_CREATOR_URI_USER"),
	...
	public static List<Order> getAllOrdersFilledByODKUser(String odkUser){
		List<Order> list = new ArrayList<Order>();
		Order order = null;
		try {
			String query = "select * from build___1354971595_core where _CREATOR_URI_USER = '"+odkUser+"'";
	...
	public static Order getOrderUsingOrderId(String _uri){
		Order order = null;
		try {
			String query = "select * from build___1354971595_core where _uri = '"+_uri+"'";
			System.out.println(query);
			ResultSet rSet = DBConnection.readFromDatabase(query);
			while(rSet.next()){
				order = new Order(rSet.getString("_URI"), rSet.getString("_CREATOR_URI_USER"),
	...
	public static void deleteThisOrderOfThisHealthPost(String _uri,int healthPostId){
		try{
			String command = "delete from build___1354971595_core where _uri = '"+_uri+"'";
			DBConnection.writeToDatabase(command);
		}catch(Exception e){
	...
	public static float getTotalOrderedAmountsForThisItemByThisUserDuringThisPeriod(String itemCode,
			String userURI,Date fromDate,Date toDate){
		float amt=0.0f;
		try{
			String query = "select sum("+itemCode+") as a from build___1354971595_core where _CREATOR_URI_USER = '"+
	...
	public static float getTotalOrderedQuantityForThisItemByThisUserDuringThisPeriod(String itemCode,
			String userURI,Date fromDate,Date toDate){
		float amt=0.0f;
		try{
			String query = "select sum("+itemCode+") as a from build___1354971595_core where _CREATOR_URI_USER = '"+
